...
class Admin extends CI_Controller {
    ...
    function reseller_create($param1 = '') {
        if ($this->admin_type == 'Master') {
            ...
            if ($param1 == 'add') {
                ...
                $num_rows = $this->db->get_where('admin', array('phone' => $phone))->num_rows();
                if($num_rows >0){
                    $data['Exist']= 'This Phone Number Already Exist';
                    $this->session->set_userdata($data);
                redirect('admin/reseller_create', 'refresh');    
                }else{
                $data['type'] = 'Reseller';
  ...
  function refun_amount($param1 = '') {
        $refund_amount = $this->uri->segment(4);

        $data1['refun_status'] = '1';
        $tr_data_by_id = $this->Common_model->get_single_data_by_single_column('transaction', 'transaction_id', $param1);
        $this->Common_model->update_data('transaction', $data1, 'transaction_id', $param1);
        
//        $tr_data_by_id = $this->Common_model->get_single_data_by_single_column('transaction', 'transaction_id', $param1);
        
        
        $data['class'] = 'In';
        $data['admin_id'] = $tr_data_by_id['admin_id'];
        $data['amount'] = $refund_amount;
        $data['created_at'] = $this->timestamp;
        
        $reseller_id = $tr_data_by_id['admin_id'];
 ...
 function rates_delete($param1 = '') {
        if ($this->admin_type == 'Master') {
            ...
            $this->meclicksasia->update_data('rates', $data, 'rates_id', $param1);
            redirect('admin/rates/' . $param1, 'refresh');
        } elseif($this->admin_type == 'Reseller') {
            redirect('reseller', 'refresh');
        } else {
            redirect('user', 'refresh');
    ...
    function comisssion_delete($param1 = '') {
        if ($this->admin_type == 'Master') {
            ...
            $this->meclicksasia->update_data('plan', $data, 'plan_id', $param1);
            redirect('admin/comissione_list/' . $param1, 'refresh');
        } elseif($this->admin_type == 'Reseller') {
            redirect('reseller', 'refresh');
        } else {
            redirect('user', 'refresh');
